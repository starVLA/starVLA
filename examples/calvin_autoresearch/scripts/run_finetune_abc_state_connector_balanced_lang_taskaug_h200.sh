...
# ABC-only post-training launcher for the 5.1+5.2 baseline.
#
# Purpose:
...
#
# It intentionally does NOT enable left/right mirror augmentation.
...
CONFIG_YAML="${CONFIG_YAML:-examples/calvin_autoresearch/train_files/starvla_gr00t_qwen3vl_action_calvin_abc_state8_connector_balanced_lang_taskaug.yaml}"
...
RUN_ID="${RUN_ID:-abc_state8_connector_balanced_lang_taskaug_ft2k_${TS}}"
...
echo "[finetune-abc-aug] main_process_port: ${MAIN_PROCESS_PORT}"

"${STARVLA_PYTHON:-python}" - "${PRETRAINED_CHECKPOINT}" "${CONFIG_YAML}" <<'PY'
import sys
...
if bad:
    for name, ok in checks.items():
        print(f"[finetune-abc-aug] check {name}: {ok}")
    raise SystemExit(f"Config/checkpoint sanity checks failed: {bad}")
print(f"[finetune-abc-aug] checkpoint sanity OK: {ckpt}")
print(f"[finetune-abc-aug] target config sanity OK: {target_cfg_path}")
PY
...
print(f"[finetune-abc-aug] visible CUDA devices: {available}")
PY
fi
...
printf '[finetune-abc-aug] command:'
printf ' %q' "${cmd[@]}"
printf '\n'
