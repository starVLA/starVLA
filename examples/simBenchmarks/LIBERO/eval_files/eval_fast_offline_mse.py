"""Offline normalized-action MSE evaluation for QwenFast on Stage 2 samples."""
...
def _load_fast_model(config_yaml: Path, checkpoint: Path, device: torch.device):
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    model, _ = _load_fast_model(args.fast_config_yaml, args.checkpoint, device)
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(tqdm(loader, desc="eval fast offline mse")):
    ...
    report = {
        "fast_config_yaml": str(args.fast_config_yaml),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate QwenFast normalized-action MSE on Stage 2 offline samples.")
    parser.add_argument("--fast_config_yaml", type=Path, required=True)
