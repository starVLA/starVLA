...
JOB_SPECS="${JOB_SPECS:?Set JOB_SPECS, e.g. '50000:libero_spatial 48000:libero_goal'}"
...
# JIHO/Jihulab training jobs often inject torchrun/accelerate distributed
# variables. Eval policy servers are independent single-process services; if
# these are inherited, accelerate tries to create a process group and every
# worker contends for the same MASTER_PORT.
...
unset ACCELERATE_USE_DEEPSPEED ACCELERATE_MIXED_PRECISION ACCELERATE_DYNAMO_BACKEND
...
start_server() {
  ...
  stop_server
  mkdir -p "${log_root}"
  SERVER_LOG="${log_root}/policy_server_gpu${GPU_ID}_port${PORT}.log"
  : > "${SERVER_LOG}"
...
summarize_ckpt() {
  ...
  if [[ -n "${suite_filter}" ]]; then
    episode_args+=(--suite "${suite_filter}" --episode-csv "${log_root}/${suite_filter}/episode_results.csv" --episode-jsonl "${log_root}/${suite_filter}/episode_results.jsonl")
  else
    episode_args+=(--episode-csv "${log_root}/libero_40task_episode_results.csv" --episode-jsonl "${log_root}/libero_40task_episode_results.jsonl")
  fi
  "${LIBERO_PYTHON}" examples/simBenchmarks/LIBERO/eval_files/summarize_libero_success.py "${log_root}" --chunked --require-ok-marker "${episode_args[@]}" | tee "${output_path}"
}
...
for job_spec in ${JOB_SPECS}; do
  ...
  suite_log_root="${log_root}/${suite}"
  worker_log="${eval_root}/worker_gpu${GPU_ID}_${suite}.log"
  progress_path="${suite_log_root}/progress.txt"
  summary_path="${log_root}/libero_40task_summary_partial_gpu${GPU_ID}_${suite}.txt"
  done_marker="${log_root}/${suite}.done"

  mkdir -p "${suite_log_root}"
  echo "[$(date)] ===== worker gpu=${GPU_ID} step=${step} suite=${suite} output=${eval_root} =====" | tee -a "${worker_log}"
  echo "[$(date)] settings seed=${EVAL_SEED} trials=${TRIALS_PER_TASK} chunk=${CHUNK_TRIALS} port=${PORT} use_bf16=${USE_BF16}" | tee -a "${worker_log}"

  start_server "${ckpt}" "${log_root}" >> "${worker_log}" 2>&1
  ...
  for task_id in $(seq 0 9); do
    ...
    while [[ "${trial_start}" -lt "${TRIALS_PER_TASK}" ]]; do
      ...
      if [[ "${SAVE_VIDEOS}" == "1" ]]; then
        ...
        video_out_path="${TMPDIR:-/tmp}/starvla_libero_eval_videos_disabled/${EVAL_OUTPUT_PREFIX}/${suite}/${ckpt_base}"
      fi
      mkdir -p "${video_out_path}"
      log_path="${suite_log_root}/${ckpt_base}_stage2_chunked_t${task_id}_r${trial_start}_n${chunk}.log"

      if chunk_completed "${log_path}" "${chunk}"; then
        echo "[$(date)] skip completed ${ckpt_base} ${suite} task=${task_id} trial=${trial_start} n=${chunk}" | tee -a "${worker_log}"
      ...
      while true; do
        if ! server_alive; then
          echo "[$(date)] server not alive before chunk; restarting" | tee -a "${worker_log}"
          start_server "${ckpt}" "${log_root}" >> "${worker_log}" 2>&1
        fi

        echo "[$(date)] eval ${ckpt_base} ${suite} task=${task_id} trial=${trial_start} n=${chunk} attempt=${attempt}" | tee -a "${worker_log}"
        if run_eval_chunk "${ckpt}" "${suite}" "${task_id}" "${trial_start}" "${chunk}" "${log_path}" "${video_out_path}" && chunk_completed "${log_path}" "${chunk}"; then
          break
        fi

        echo "[$(date)] chunk failed or incomplete: ${log_path}" | tee -a "${worker_log}"
        if ! server_alive; then
          echo "[$(date)] server died after failed chunk; restarting" | tee -a "${worker_log}"
          start_server "${ckpt}" "${log_root}" >> "${worker_log}" 2>&1
        fi
        if [[ "${attempt}" -ge "${MAX_RETRIES}" ]]; then
          echo "[$(date)] failed after ${MAX_RETRIES} attempts: ${ckpt_base} ${suite} task=${task_id} trial=${trial_start}" | tee -a "${worker_log}" >&2
          exit 1
        fi
        attempt=$((attempt + 1))
        sleep 5
...

    {
      ...
      date
      summarize_ckpt "${log_root}" "${progress_path}" "${suite}"
      echo
    } | tee -a "${worker_log}"
  done

  summarize_ckpt "${log_root}" "${summary_path}" "${suite}" >> "${worker_log}" 2>&1 || true
  touch "${done_marker}"
  echo "[$(date)] ===== completed worker job step=${step} suite=${suite}; marker=${done_marker} =====" | tee -a "${worker_log}"
  stop_server
done

echo "[$(date)] worker gpu=${GPU_ID} completed jobs: ${JOB_SPECS}"
