...
CONFIG_YAML="${CONFIG_YAML:-examples/simBenchmarks/Robocasa_tabletop/train_files/train_var_stage1_robocasa_gr1_abs_productvq_g16_s1_2_4_8_16_weighted.yaml}"
INIT_CKPT="/root/feihong/starVLA/Checkpoints/var_stage1_robocasa_gr1_abs_pure_ae_e128_weighted/best_recon.ckpt"
if [[ ! -f "${INIT_CKPT}" ]]; then
  echo "Missing abs weighted pure AE init checkpoint: ${INIT_CKPT}" >&2
  exit 1
fi
