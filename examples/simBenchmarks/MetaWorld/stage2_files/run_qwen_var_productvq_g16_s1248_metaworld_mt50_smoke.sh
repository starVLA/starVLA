...
RUN_DIR="/root/nas/feihong/starVLA/Checkpoints/qwen_var_productvq_g16_s1248_metaworld_mt50_smoke"
mkdir -p "${RUN_DIR}"
...
"${PYTHON_BIN}" -m accelerate.commands.launch \
  ...
  --main_process_port "${MAIN_PROCESS_PORT:-29549}" \
  starVLA/training/train_starvla.py \
  --config_yaml examples/simBenchmarks/MetaWorld/stage2_files/train_qwen_var_productvq_g16_s1248_metaworld_mt50_smoke.yaml \
  2>&1 | tee -a "${LOG_FILE}"
