...
class GradientAccumulationConfigTest(unittest.TestCase):
    def test_shared_builder_forwards_configured_accumulation_steps(self):
        ...
        cfg = SimpleNamespace(trainer=SimpleNamespace(gradient_accumulation_steps=4, gradient_clipping=1.0))
        ...
        plugin_cls.assert_called_once_with(gradient_clipping=1.0)
    ...
    def test_shared_builder_preserves_deepspeed_opt_out(self):
        trainer_tools = importlib.import_module("starVLA.training.trainer_utils.trainer_tools")
        cfg = SimpleNamespace(trainer=SimpleNamespace(gradient_accumulation_steps=4, gradient_clipping=1.0))
        accelerator = mock.Mock(state=object())

        with (
            mock.patch.dict("os.environ", {"STARVLA_DISABLE_DEEPSPEED": "1"}),
            mock.patch.object(trainer_tools, "DeepSpeedPlugin") as plugin_cls,
            mock.patch.object(trainer_tools, "Accelerator", return_value=accelerator) as accelerator_cls,
        ):
            trainer_tools.build_accelerator(cfg)

        plugin_cls.assert_not_called()
        accelerator_cls.assert_called_once_with(gradient_accumulation_steps=4, deepspeed_plugin=None)
