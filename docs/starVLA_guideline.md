...
Please check the value of `data_root_dir` in `starvla_cotrain_libero.yaml` config file is `playground/Datasets/LEROBOT_LIBERO_DATA`.
<details>
...
The potential solution is usa `starvla_cotrain_libero.yaml` rather `starvla_cotrain_robotwin.yaml`. For example, 
```python
parser.add_argument(
    "--config_yaml",
    type=str,
