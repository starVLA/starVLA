...
def _build_input_features(config) -> dict[str, PolicyFeature]:
    ...
    features = {
        _image_feature_name(image_key): PolicyFeature(type=FeatureType.VISUAL, shape=(3, image_height, image_width))
        for image_key in image_keys
    }
    features[OBS_STATE] = PolicyFeature(type=FeatureType.STATE, shape=(int(config.state_dim),))
    return features


def _build_output_features(config) -> dict[str, PolicyFeature]:
    return {ACTION: PolicyFeature(type=FeatureType.ACTION, shape=(int(config.action_dim),))}
...
class ACT(baseframework):
    def __init__(self, config, **kwargs):
        ...
        act_config = ACTConfig(
            ...
            pretrained_backbone_weights=framework_config.get("pretrained_backbone_weights", None),
        )
    ...
    def _set_identity_normalizers(self) -> None:
        ...
        with torch.no_grad():
            ...
            for normalizer in normalizers:
                tensors = list(normalizer.named_buffers()) + list(normalizer.named_parameters())
    ...
    def _examples_to_act_batch(self, examples: List[dict], require_action: bool = True) -> dict:
        framework_config = self.config.framework
        image_keys = list(framework_config.get("image_keys", ACTDefaultConfig.image_keys))
        chunk_size = int(framework_config.chunk_size)
        ...
        for view_idx, image_key in enumerate(image_keys):
            ...
            # otherwise retain the backward-compatible PIL path.
            if isinstance(examples[0]["image"][view_idx], np.ndarray):
                arr = np.stack([example["image"][view_idx] for example in examples], axis=0)
            ...
            for example in examples:
                image = example["image"][view_idx].convert("RGB")
                if image.size != target_image_size:
                    image = image.resize(target_image_size)
                image_array = np.asarray(image, dtype=np.float32) / 255.0
                image_tensor = torch.from_numpy(image_array).permute(2, 0, 1).to(device=device, dtype=model_dtype)
        ...
        for example in examples:
            ...
            state_tensor = torch.as_tensor(state, dtype=model_dtype, device=device).reshape(-1)
            if state_tensor.numel() != state_dim:
                raise ValueError(f"ACT state must have {state_dim} values, got {state_tensor.numel()}")
        ...
        for example in examples:
            ...
            if action_tensor.ndim != 2 or action_tensor.shape[-1] != action_dim:
                raise ValueError(f"ACT action must have shape [T, {action_dim}], got {tuple(action_tensor.shape)}")
            if action_tensor.shape[0] < chunk_size:
                raise ValueError(f"ACT action horizon must be at least {chunk_size}, got {action_tensor.shape[0]}")
            actions.append(action_tensor[-chunk_size:])
        batch[ACTION] = torch.stack(actions, dim=0)
        batch["action_is_pad"] = torch.zeros(len(examples), chunk_size, dtype=torch.bool, device=device)
    ...
    def predict_action(self, examples: List[dict], **kwargs) -> dict:
        ...
        model_param = next(self.action_model.parameters())
        autocast_enabled = model_param.dtype in (torch.float16, torch.bfloat16) and model_param.device.type in (
            "cuda",
            "cpu",
        )
        try:
