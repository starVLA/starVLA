...
def _is_robocasa_gr1_dataset(dataset_name: str) -> bool:
    return (
        dataset_name.startswith("gr1_unified.")
        and (
            dataset_name.endswith("_GR1ArmsAndWaistFourierHands_1000")
            or dataset_name.endswith("_GR1ArmsAndWaistFourierHands")
        )
    )
...
def _task_name_from_metadata(metadata: dict[str, Any]) -> str:
    ...
    dataset_task_name = _simplify_dataset_name(dataset_name)
    # RoboCasa's 24 datasets are the task contract used by epochs 0..47.
    # The shared loader may also expose a generic task_index column that is
    # needed by LIBERO, but for RoboCasa it must not split one dataset into
    # synthetic dataset::task_XX keys.
    if _is_robocasa_gr1_dataset(dataset_name):
        return dataset_task_name
    explicit_task_name = str(metadata.get("task_name", "")).strip()
    if explicit_task_name:
        return explicit_task_name
    return dataset_task_name or "unknown"
...
def build_static_task_balance_weights(dataset: Any, cfg: Any) -> dict[str, float]:
    ...
    if sources:
        ...
        for dataset_index, source in enumerate(sources):
            raw_dataset_name = str(getattr(source, "dataset_name", dataset_index))
            dataset_name = _simplify_dataset_name(raw_dataset_name)
            is_robocasa_task = _is_robocasa_gr1_dataset(raw_dataset_name)
                ...
                if not is_robocasa_task:
                    try:
                        ...
                        if "task_index" in trajectory_data.columns:
                            ...
                            suite_name = raw_dataset_name.rsplit("/", 1)[-1]
...
def train(cfg: Any) -> None:
    ...
    guard_cfg = cfg.train.get("task_mae_guardrail", None)
    guard_enabled = bool(guard_cfg is not None and guard_cfg.get("enabled", False))
    baseline_task_mae: dict[str, float] = {}
    baseline_task_counts: dict[str, int] = {}
    guardrail_stop_record: dict[str, Any] | None = None
    if guard_enabled:
        expected_tasks = int(guard_cfg.get("expected_tasks", 24))
        tail_k = int(guard_cfg.get("tail_k", 4))
        smoothing_window = int(guard_cfg.get("smoothing_window", 3))
        if expected_tasks <= 0 or tail_k <= 0 or tail_k > expected_tasks or smoothing_window <= 0:
            raise ValueError(
                "Invalid task MAE guardrail dimensions: "
                f"expected_tasks={expected_tasks}, tail_k={tail_k}, smoothing_window={smoothing_window}."
            )
        baseline_path = Path(str(guard_cfg.get("baseline_task_metrics_path")))
        ...
        baseline_task_counts = {
            str(name): int(record["count"])
            for name, record in baseline_payload["tasks"].items()
        }
        if any(count <= 0 for count in baseline_task_counts.values()):
            raise ValueError(f"Invalid per-task counts in guardrail baseline {baseline_path}.")
        if history:
            previous_guardrail = history[-1].get("task_mae_guardrail")
            if isinstance(previous_guardrail, dict) and bool(previous_guardrail.get("should_stop", False)):
                raise RuntimeError(
                    "The resume checkpoint already contains a terminal task-MAE guardrail decision "
                    f"at epoch {history[-1].get('epoch')}; refusing to train further."
                )
        print(
            "Enabled task-MAE guardrail: "
            f"{expected_tasks} tasks, baseline={baseline_path}, "
            f"patience={int(guard_cfg.get('patience', 8))}."
        )

    adaptive_task_weights_path = output_dir / "adaptive_task_weights.json"
    task_error_weights: dict[str, float] = {}
    embedded_adaptive_weights = history[-1].get("adaptive_task_weights") if history else None
    if isinstance(embedded_adaptive_weights, dict) and embedded_adaptive_weights:
        task_error_weights = {
            str(key): float(value)
            for key, value in embedded_adaptive_weights.items()
        }
        print(f"Loaded adaptive task weights embedded in checkpoint epoch {history[-1]['epoch']}.")
    elif adaptive_task_weights_path.exists():
        try:
    ...
    for epoch in range(start_epoch, int(cfg.train.epochs)):
        ...
        if reconstruction_stats is not None:
            ...
            epoch_record["adaptive_task_weights"] = {
                str(key): float(value)
                for key, value in task_error_weights.items()
            }

        guardrail_record = None
        if guard_enabled:
            if reconstruction_summary is None:
                raise RuntimeError("Task-MAE guardrail requires per-task reconstruction statistics.")
            current_task_counts = {
                str(name): int(record["count"])
                for name, record in reconstruction_summary.get("tasks", {}).items()
            }
            count_mismatches = {
                name: (baseline_task_counts.get(name), current_task_counts.get(name))
                for name in sorted(set(baseline_task_counts) | set(current_task_counts))
                if baseline_task_counts.get(name) != current_task_counts.get(name)
            }
            if count_mismatches:
                raise RuntimeError(
                    "Per-task sample counts changed relative to epoch-47 baseline: "
                    f"{count_mismatches}"
                )
            guardrail_record = evaluate_stage1_task_mae_guardrail(
                epoch=epoch,
                history=history,
                current_epoch_record=epoch_record,
                reconstruction_summary=reconstruction_summary,
                baseline_task_mae=baseline_task_mae,
                codebook_usage_ratio=usage_ratio,
                cfg=guard_cfg,
            )
            epoch_record["task_mae_guardrail"] = guardrail_record
        ...
        if guardrail_record is not None:
            ...
            print(
                "Task-MAE guardrail: "
                f"epoch={epoch}, raw_macro={guardrail_record['task_mae_mean']:.9g}, "
                f"median3={guardrail_record['task_mae_mean_smoothed']:.9g}, "
                f"best={guardrail_record['best_smoothed_task_mae_mean']:.9g}, "
                f"without_improvement={guardrail_record['epochs_without_meaningful_improvement']}, "
                f"stop={guardrail_record['should_stop']}."
            )
            if bool(guardrail_record.get("meaningful_new_best", False)):
                save_checkpoint(
                    output_dir / "best_task_mae_guardrail.ckpt",
                    model=model,
                    optimizer=optimizer,
                    epoch=epoch,
                    history=history,
            ...
            if bool(guardrail_record.get("should_stop", False)):
                guardrail_stop_record = guardrail_record
                print(f"Task-MAE guardrail requested a clean stop: {guardrail_record['stop_reasons']}")
                break
    ...
    if guardrail_stop_record is not None:
        save_json(
            output_dir / "STOPPED_EARLY.json",
            {
                "epoch": int(history[-1]["epoch"]),
                "final_checkpoint": str(output_dir / "final.ckpt"),
                "guardrail": guardrail_stop_record,
            },
        )
    else:
        save_json(
