...
NUM_PROCESSES="8"
# === FINE-TUNE ADJUSTMENT: point at the new yaml ===
CONFIG_YAML="${CONFIG_YAML:-examples/calvin/train_files/starvla_train_calvin_abc_qwen35_2b_gr00t_multiview_d_style_ft.yaml}"
...
# === FINE-TUNE ADJUSTMENT: unique run-id prefix so it does NOT collide with the 40k from-scratch run ===
RUN_ID_PREFIX="${RUN_ID_PREFIX:-qwen35_2b_gr00t_calvin_abc_multiview_d_style_ft}"

RUN_TIMESTAMP="${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
RUN_ID="${RUN_ID:-${RUN_ID_PREFIX}_${RUN_TIMESTAMP}}"

echo "===== StarVLA Qwen3.5 multiview D-style FINE-TUNE launch ====="
